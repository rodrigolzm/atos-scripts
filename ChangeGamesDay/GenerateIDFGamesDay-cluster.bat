...
rem gcm -module FailoverClusters

call :logtime "INFO  Stopping IDF Dispatcher..."

powershell -c "Get-ClusterGroup 'MEV Role' | Get-ClusterResource 'MEV IDF' | Stop-ClusterResource"
if errorlevel 1 (
  call :logtime "ERROR trying to stop Resource 'MEV IDF' of cluster group 'MEV Role'"
  exit /b 1
)
...
powershell -c "Get-ClusterGroup 'MEV Role' | Get-ClusterResource 'MEV IDF' | Start-ClusterResource"
if errorlevel 1 (
  call :logtime "ERROR trying to stop Resource 'MEV IDF' of cluster group 'MEV Role'"
  exit /b 2
)
